...
public sealed class GetAllPatientTasksForPatientHandler(IPatientTaskRepository repository)
    : IRequestHandler<GetAllPatientTasksForPatientQuery, IReadOnlyList<PatientTaskResponse>>
{
    public async Task<IReadOnlyList<PatientTaskResponse>> Handle(
        GetAllPatientTasksForPatientQuery request,
        CancellationToken cancellationToken)
    {
        
        var tasks = (await repository.GetAllForPatientAsync(request.PatientId, cancellationToken)).ToList();
        if (tasks.Count == 0)
